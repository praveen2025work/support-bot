...
export default function TrendChart({
  ...
  const trendColor = DIRECTION_COLORS[direction];

  const combinedData = useMemo(() => {
    const all = new Map<number, { x: number; actual?: number; trend?: number; label?: string }>();
  ...
  return (
    <div className="mt-3 border border-gray-200 rounded-lg p-2 bg-white">
      <div className="flex items-center justify-between mb-1">
        <span className="text-xs font-medium text-gray-600">Trend Analysis</span>
        <span
      ...
      <div className="flex items-center gap-4 mt-1 text-[10px] text-gray-500">
        <div>
          Slope: <span className="font-medium text-gray-700">{slope.toFixed(4)}</span>
        </div>
        <div>
          R&sup2;: <span className="font-medium text-gray-700">{rSquared.toFixed(4)}</span>
        </div>
      </div>
    </div>
  );
