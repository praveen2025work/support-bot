  ...
  return (
    ...
    >
      ...
      >
        ...
        {isLeaf ? (
          <span className="bg-green-50 border border-green-200 rounded px-1.5 py-0.5">
            Predict:{" "}
            <span className="font-medium">{node.prediction ?? "?"}</span>
          ...
          <span className="bg-gray-50 border border-gray-200 rounded px-1.5 py-0.5 text-gray-400 italic">
            {node.feature} &le; {node.threshold?.toFixed(2)} ... (truncated, n=
            {node.samples})
          </span>
        ) : (
      ...
      {!isLeaf && !isTruncated && (
        <>
          {node.left && (
            <TreeNodeView node={node.left} depth={depth + 1} path="L" />
          )}
          {node.right && (
            <TreeNodeView node={node.right} depth={depth + 1} path="R" />
          )}
        </>
      )}
    </div>
...
export default function DecisionTreeViz({
  ...
  return (
    <div className="mt-3 border border-gray-200 rounded-lg p-2 bg-white">
      ...
      {/* Tree structure */}
      <div
        className="overflow-auto border border-gray-100 rounded p-1 mb-2 bg-gray-50"
        style={{ maxHeight: 250 }}
      >
      ...
      {importanceData.length > 0 && (
        <>
          ...
          <ResponsiveContainer
            width="100%"
            height={Math.max(120, importanceData.length * 24)}
          >
            <BarChart
              data={importanceData}
              layout="vertical"
            ...
            >
              ...
              <Bar dataKey="importance" radius={[0, 2, 2, 0]}>
                {importanceData.map((_, i) => {
                  const opacity =
                    0.4 +
                    0.6 * (1 - i / Math.max(importanceData.length - 1, 1));
                  return (
