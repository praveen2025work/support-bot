...
export function SimpleGrid({
  ...
  const effectiveCols = containerWidth < 480 ? 2 : containerWidth < 768 ? 4 : containerWidth < 996 ? 8 : cols;
  const colWidth = containerWidth > 0 ? (containerWidth - gap * (effectiveCols - 1)) / effectiveCols : 0;
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const newW = Math.max(minW, Math.min(effectiveCols, resizing.startW + deltaCols));
    ...
    const handleMouseUp = () => {
      if (resizePreview) {
        const newLayouts = layouts.map((l) =>
          l.i === resizing.id ? { ...l, w: resizePreview.w, h: resizePreview.h } : l
        );
        onLayoutChange?.(newLayouts);
      }
      setResizing(null);
  ...
  }, [resizing, resizePreview, layouts, onLayoutChange, colWidth, gap, rowHeight, effectiveCols]);
  ...
  return (
    ...
    >
      {sorted.map((item) => {
        ...
        const previewW = resizePreview?.id === item.i ? resizePreview.w : item.w;
        const previewH = resizePreview?.id === item.i ? resizePreview.h : item.h;
        ...
        return (
          <div
            ...
            style={{
              ...
              outline: isDropTarget ? '2px dashed #3b82f6' : isResizing ? '2px solid #3b82f6' : 'none',
              outlineOffset: '2px',
