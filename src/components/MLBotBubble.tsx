...
function loadChartJs(): Promise<void> {
  ...
  if (chartJsFailed) return Promise.reject(new Error('Chart.js failed to load'));
...
function InlineChart({ config }: { config: MLBotMessage['chart'] }) {
  ...
  useEffect(() => {
    ...
    loadChartJs().then(() => {
      const Chart = (window as unknown as { Chart: new (el: HTMLCanvasElement, cfg: unknown) => { destroy(): void } }).Chart;
      if (!Chart) {
      ...
      const COLORS = ['#185FA5', '#1D9E75', '#D85A30', '#BA7517', '#993556', '#3C3489'];
      ...
      chartRef.current = new Chart(canvasRef.current!, {
        ...
        options: {
          ...
          scales: ['bar', 'line', 'scatter'].includes(config.type) ? {
            x: { grid: { color: 'rgba(128,128,128,0.1)' }, ticks: { font: { size: 10 }, maxTicksLimit: 8 } },
...
function InlineTable({ headers, rows }: { headers: string[]; rows: (string | number)[][] }) {
    ...
    <div style={{ marginTop: 12, overflowX: 'auto' }}>
        ...
        <tbody>
          {displayRows.map((row, i) => (
            <tr key={i} style={{ background: i % 2 === 0 ? 'transparent' : 'rgba(128,128,128,0.04)' }}>
              {row.map((cell, j) => (
                <td key={j} style={{ padding: '4px 10px', color: 'var(--color-text-primary)' }}>
                  {String(cell)}
                </td>
              ))}
            </tr>
      ...
      {rows.length > 5 && (
        <button
          ...
          style={{
            marginTop: 6, fontSize: 11, background: 'none', border: 'none',
...
const ANALYSIS_META: Record<string, { icon: string; color: string; bg: string }> = {
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
        <span style={{
          ...
          background: meta.bg, color: meta.color, fontSize: 13, fontWeight: 500,
        }}>
          {meta.icon}
        </span>
      ...
      {message.details.length > 0 && (
        <ul style={{ margin: '0 0 10px 0', paddingLeft: 18, fontSize: 13, lineHeight: 1.7 }}>
          {message.details.map((d, i) => (
            <li key={i} style={{ color: 'var(--color-text-secondary)', marginBottom: 2 }}>{d}</li>
          ))}
        </ul>
      )}
      ...
      {/* Data table */}
      {message.tableData && (
        <InlineTable headers={message.tableData.headers} rows={message.tableData.rows} />
      )}
