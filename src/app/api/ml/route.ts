  ...
  try {
    ...
    if (!body.nlpResult || !body.rawQuery || !body.csvData) {
      return NextResponse.json(
        {
          success: false,
          error: "Missing required fields: nlpResult, rawQuery, csvData",
        },
        { status: 400 },
      );
    }
    ...
    if (
      tenantConfig.maxFileSize > 0 &&
      csvByteSize > tenantConfig.maxFileSize
    ) {
      return NextResponse.json(
        {
          success: false,
          error: `File size (${(csvByteSize / 1024 / 1024).toFixed(1)}MB) exceeds limit (${(tenantConfig.maxFileSize / 1024 / 1024).toFixed(0)}MB)`,
        },
        { status: 413 },
      );
    }
    ...
    if (tenantConfig.maxRows > 0 && rowCount > tenantConfig.maxRows) {
      return NextResponse.json(
        {
          success: false,
          error: `Row count (${rowCount.toLocaleString()}) exceeds limit (${tenantConfig.maxRows.toLocaleString()})`,
        },
        { status: 413 },
      );
    }
    ...
    // Build intent
    const intent = buildParsedIntent(
      body.nlpResult,
      body.rawQuery,
      columnNames,
    );
    ...
    if (
      analysisType &&
      tenantConfig.allowedAnalyses.length > 0 &&
      !tenantConfig.allowedAnalyses.includes(analysisType)
    ) {
      return NextResponse.json(
        {
          success: false,
          error: `Analysis type "${analysisType}" is not enabled for tenant "${tenantConfig.tenantId}"`,
        },
        { status: 403 },
      );
    }
