...
// GET: List connectors OR get single connector details/schemas/tables/columns
export async function GET(request: NextRequest) {
  try {
    const id = request.nextUrl.searchParams.get("id");

    if (!id) {
      const response = await proxyToConnector("csv-xlsx", "/api/connectors");
      const data = await response.json();
      return NextResponse.json(data, { status: response.status });
    }

    const resource = request.nextUrl.searchParams.get("resource");
    const schema = request.nextUrl.searchParams.get("schema") || "default";
    ...
    logger.error({ error }, "Failed to proxy GET /csv-connector");
    return NextResponse.json(
      { error: "Failed to fetch CSV connectors" },
      { status: 500 },
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    logger.error({ error }, "Failed to proxy POST /csv-connector");
    return NextResponse.json(
...
export async function PUT(request: NextRequest) {
  try {
    ...
    logger.error({ error }, "Failed to proxy PUT /csv-connector");
    return NextResponse.json(
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    logger.error({ error }, "Failed to proxy DELETE /csv-connector");
    return NextResponse.json(
