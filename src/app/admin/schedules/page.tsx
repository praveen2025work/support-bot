...
export default function SchedulesPage() {
  ...
  const fetchQueries = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        setAvailableQueries((data.queries || []).map((q: any) => ({ name: q.name, source: q.source })));
      }
    } catch {
      // ignore
    }
  ...
  function openEdit(s: ScheduledQuery) {
    ...
    const matchingPreset = CRON_PRESETS.find((p) => p.value === s.cronExpression);
    ...
    setFormFilters(Object.keys(s.filters).length > 0 ? JSON.stringify(s.filters) : '');
    setShowForm(true);
    setError('');
  }
    ...
    try {
      if (editingId) {
        const res = await fetch(`/api/admin/schedules/${editingId}`, {
          ...
          body: JSON.stringify({ cronExpression, filters, label: formLabel || formQueryName, enabled: true }),
        });
        if (!res.ok) {
          const data = await res.json();
  ...
  return (
    <div className="max-w-6xl">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-[var(--text-primary)]">Scheduled Queries</h1>
          <p className="text-sm text-[var(--text-muted)]">Manage recurring query execution schedules</p>
        </div>
        <div className="flex gap-2">
          <button onClick={fetchSchedules} className="text-xs text-[var(--text-link,#2563eb)] hover:underline">Refresh</button>
          <button
            onClick={openCreate}
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="bg-[var(--bg-card)] rounded-lg border border-[var(--border-primary)] p-4">
          <div className="text-xs text-[var(--text-muted)] uppercase tracking-wide">Total Schedules</div>
          <div className="text-2xl font-bold text-[var(--text-primary)] mt-1">{schedules.length}</div>
        </div>
        <div className="bg-[var(--bg-card)] rounded-lg border border-[var(--border-primary)] p-4">
        ...
        <div className="bg-[var(--bg-card)] rounded-lg border border-[var(--border-primary)] p-4">
          <div className="text-xs text-[var(--text-muted)] uppercase tracking-wide">Total Runs</div>
          <div className="text-2xl font-bold text-[var(--text-primary)] mt-1">{totalRuns}</div>
        </div>
      </div>

      {/* Create / Edit form */}
      {showForm && (
        <div className="bg-[var(--bg-card)] rounded-lg border border-[var(--border-primary)] p-5 mb-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              {/* Query name */}
              <div>
                <label className="block text-xs font-medium text-[var(--text-muted)] mb-1">Query</label>
                {editingId ? (
                  <input
                    value={formQueryName}
                    disabled
              ...
              {!editingId && (
                <div>
                  <label className="block text-xs font-medium text-[var(--text-muted)] mb-1">Group</label>
                  <input
                    value={formGroupId}
      ...
      {/* Table */}
      {loading ? (
        <div className="text-sm text-[var(--text-muted)]">Loading schedules...</div>
      ) : schedules.length === 0 ? (
        <div className="bg-[var(--bg-card)] rounded-lg border border-[var(--border-primary)] p-8 text-center">
          <p className="text-sm text-[var(--text-muted)]">No scheduled queries yet.</p>
          <button
            onClick={openCreate}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[var(--border-primary)] bg-[var(--bg-card)]">
                <th className="text-left px-4 py-2.5 text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wide">Query</th>
                <th className="text-left px-4 py-2.5 text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wide">Schedule</th>
                <th className="text-left px-4 py-2.5 text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wide">Next Run</th>
                <th className="text-left px-4 py-2.5 text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wide">Last Run</th>
                <th className="text-left px-4 py-2.5 text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wide">Runs</th>
                <th className="text-center px-4 py-2.5 text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wide">Enabled</th>
                <th className="text-right px-4 py-2.5 text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wide">Actions</th>
              </tr>
            </thead>
            <tbody>
              {schedules.map((s) => (
                <tr key={s.id} className="border-b border-[var(--border-primary)] last:border-0 hover:bg-[var(--bg-card-hover,var(--bg-card))]">
                  <td className="px-4 py-3">
                    <div className="font-medium text-[var(--text-primary)]">{s.label}</div>
                    {s.label !== s.queryName && (
                      <div className="text-xs text-[var(--text-muted)]">{s.queryName}</div>
                    )}
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3 text-[var(--text-muted)] text-xs">{s.enabled ? formatDate(s.nextRunAt) : '--'}</td>
                  <td className="px-4 py-3 text-[var(--text-muted)] text-xs">{formatDate(s.lastRunAt)}</td>
                  <td className="px-4 py-3 text-[var(--text-muted)] text-xs">{s.runCount}</td>
