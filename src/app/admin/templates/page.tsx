...
export default function TemplateEditorPage() {
  ...
  const currentTemplates =
    activeScope === "base" ? baseTemplates : groupTemplates[activeScope] || {};
  ...
  const handleDelete = async (intent: string) => {
    await fetch(
      `/api/admin/templates?scope=${activeScope}&intent=${encodeURIComponent(intent)}`,
    ...
    <div>
      <div className="pb-6 mb-6 border-b border-gray-100">
        ...
        <p className="text-sm text-gray-500">
          Manage response templates for bot intents. Group-specific templates
            ...
            {intentKeys.map((intent) => (
              ...
              >
                ...
                <div className="text-xs text-gray-400">
                  {currentTemplates[intent]?.length || 0} response
                  {currentTemplates[intent]?.length !== 1 ? "s" : ""}
                </div>
              </button>
            ))}
            {intentKeys.length === 0 && (
              <p className="text-xs text-gray-400 px-3 py-2">
                {activeScope === "base"
                  ? "No base templates defined"
                  : "No overrides for this group"}
              </p>
            )}
          </div>
        </div>
          ...
          {showNew ? (
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  onClick={() => setDeleteTemplateTarget(selectedIntent)}
              ...
              <div className="mb-4">
                ...
                />
                <p className="text-xs text-gray-400 mt-1">
                  The bot randomly selects one of these responses when this
                  intent is matched.
                </p>
              </div>
                ...
                {editResponses
                  ...
                  .map((resp, i) => (
                    <div key={i} className="mb-2 last:mb-0">
                      ...
                      <div className="text-sm text-gray-700 bg-white rounded p-2 mt-0.5 whitespace-pre-wrap">
                        {resp.trim()}
                      </div>
                    </div>
                  ))}
      ...
      <ConfirmModal
        ...
        message={
          deleteTemplateTarget
            ? `Delete template "${deleteTemplateTarget}" from ${activeScope}?`
            : ""
        }
        confirmLabel="Delete"
        variant="danger"
        onConfirm={() => {
          if (deleteTemplateTarget) handleDelete(deleteTemplateTarget);
        }}
