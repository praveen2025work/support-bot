    ...
    <div>
      <div className="flex items-center justify-between pb-6 mb-6 border-b border-gray-100">
        <div>
          ...
          <p className="text-sm text-gray-500">
            {total} total conversations logged
          </p>
        </div>
            ...
            {Object.entries(intentDist)
              ...
              .map(([intent, count]) => (
                <button
                  key={intent}
                  onClick={() =>
                    setFilterIntent(filterIntent === intent ? "" : intent)
                  }
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 text-center">
          <p className="text-sm text-gray-500">
            No conversation logs yet. Chat with the bot to generate logs.
          </p>
        </div>
      ) : (
            ...
            <table className="w-full text-sm">
              <thead className="sticky top-0 bg-gray-50/80 z-10">
                <tr className="border-b border-gray-200">
                  <th className="text-left px-4 py-2 text-xs font-medium text-gray-500 uppercase tracking-wider w-36">
                    Time
                  </th>
                  <th className="text-left px-4 py-2 text-xs font-medium text-gray-500 uppercase tracking-wider">
                    User Message
                  </th>
                  <th className="text-left px-4 py-2 text-xs font-medium text-gray-500 uppercase tracking-wider w-28">
                    Intent
                  </th>
                  <th className="text-right px-4 py-2 text-xs font-medium text-gray-500 uppercase tracking-wider w-20">
                    Conf.
                  </th>
                  <th className="text-left px-4 py-2 text-xs font-medium text-gray-500 uppercase tracking-wider w-20">
                    Group
                  </th>
                </tr>
              </thead>
                        ...
                        <td colSpan={5} className="px-4 py-3">
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              ...
                              <div className="text-sm text-gray-800 bg-white rounded p-2 border border-gray-200">
                                {log.userMessage}
                              </div>
                            </div>
                            <div>
                              ...
                              <div className="text-sm text-gray-800 bg-white rounded p-2 border border-gray-200 whitespace-pre-wrap max-h-32 overflow-y-auto">
                                {log.botResponse}
                              </div>
                            </div>
                          </div>
                          <div className="flex items-center gap-4 mt-2 text-xs text-gray-400">
                            ...
                            {log.executionMs != null && (
                              <span>Execution: {log.executionMs}ms</span>
                            )}
                            {log.hasRichContent && (
