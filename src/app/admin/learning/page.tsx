...
export default function LearningPage() {
  ...
  const fetchReviewQueue = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/admin/learning/review?groupId=${groupId}&status=pending`,
      );
  ...
  const fetchAutoLearned = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/admin/learning/auto-learned?groupId=${groupId}`,
      );
  ...
  const fetchIntents = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        ...
        const intentList = (data.intents || data).map(
          (i: { intent: string }) => i.intent,
        );
  ...
  const triggerRetrain = async () => {
    try {
      ...
      if (res.ok) {
        setStatus(
          "NLP retrain triggered — next message will use updated model",
        );
  ...
  const processSignals = async () => {
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        setStatus(
          `Processed: ${data.promoted} promoted, ${data.queued} still queued`,
        );
        setTimeout(() => setStatus(""), 3000);
  ...
  return (
    <div>
      <div className="flex items-center justify-between pb-6 mb-6 border-b border-gray-100">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">Learning</h1>
          <p className="text-sm text-gray-500">
            NLP self-improvement from user interactions
          </p>
        </div>
        <select
          value={groupId}
          ...
          {loading ? (
            ...
            <div className="text-center py-12 text-gray-400">
              ...
              <p className="text-sm mt-1">
                Low-confidence messages will appear here for labeling
              </p>
            </div>
          ) : (
                ...
                >
                  ...
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                      ...
                      <span
                        className={`text-xs font-medium ${item.confidence < 0.3 ? "text-red-600" : "text-yellow-600"}`}
                      >
        ...
        <div>
          {loading ? (
            ...
            <div className="text-center py-12 text-gray-400">
              ...
              <p className="text-sm mt-1">
                Utterances will appear here once promoted from feedback signals
              </p>
            </div>
          ) : (
            <table className="w-full text-sm table-fixed">
              <thead>
                <tr className="border-b border-gray-200 text-left text-gray-500">
                  ...
                  <th className="pb-2 font-medium w-[10%] text-center">
                    Signals
                  </th>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-4 gap-4">
                {[
                  ...
                  {
                    label: "Auto-Learned",
                    value: stats.autoLearned,
                    color: "text-green-600",
                  },
                  {
                    label: "Admin-Resolved",
                    value: stats.resolvedByAdmin,
                    color: "text-blue-600",
                  },
                ].map((card) => (
                  <div
                ...
                <div className="flex items-end gap-1 h-24">
                  {stats.recentActivity.map((day) => {
                    const maxInteractions = Math.max(
                      ...stats.recentActivity.map((d) => d.interactions),
                      1,
                    );
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="px-4 py-2 bg-blue-600 text-white text-sm rounded-md shadow-sm hover:bg-blue-700"
                >
                  Retrain NLP
                </button>
                <button
