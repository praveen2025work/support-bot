...
export default function AuditTrailPage() {
  ...
  const actionsToday = entries.filter((e) =>
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="bg-[var(--bg-card)] rounded-xl shadow-sm border border-[var(--border-primary)] p-8 text-center">
          <p className="text-sm text-[var(--text-muted)]">
            No audit entries yet. Actions will be logged as users interact with
            the admin panel.
          </p>
        </div>
      ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-[var(--bg-secondary)] border-b border-[var(--border-primary)]">
                  <th className="text-left px-4 py-2 text-xs font-medium text-[var(--text-tertiary)] uppercase tracking-wider w-40">
                    Timestamp
                  </th>
                  <th className="text-left px-4 py-2 text-xs font-medium text-[var(--text-tertiary)] uppercase tracking-wider w-24">
                    Action
                  </th>
                  <th className="text-left px-4 py-2 text-xs font-medium text-[var(--text-tertiary)] uppercase tracking-wider w-28">
                    User
                  </th>
                  <th className="text-left px-4 py-2 text-xs font-medium text-[var(--text-tertiary)] uppercase tracking-wider w-24">
                    Type
                  </th>
                  <th className="text-left px-4 py-2 text-xs font-medium text-[var(--text-tertiary)] uppercase tracking-wider">
                    Resource
                  </th>
                  <th className="text-left px-4 py-2 text-xs font-medium text-[var(--text-tertiary)] uppercase tracking-wider">
                    Details
                ...
                {paged.map((entry) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-2">
                      <span
                        className={`inline-block px-2 py-0.5 rounded text-xs font-medium ${actionColors[entry.action] || "bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300"}`}
                      >
                        {entry.action}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-2 text-xs text-[var(--text-primary)] font-mono">
                      {entry.resource}
                    </td>
                    <td className="px-4 py-2 text-xs text-[var(--text-muted)] truncate max-w-xs">
                      {entry.details || "\u2014"}
                    </td>
                  </tr>
                ))}
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between mt-4">
              <span className="text-xs text-[var(--text-muted)]">
                ...
                {Math.min((page + 1) * PAGE_SIZE, entries.length)} of{" "}
                {entries.length}
              </span>
