    ...
    <div className="mx-auto max-w-4xl">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500">
          Deploy UI + Engine to production — no Mock API needed
        </p>
...
└──────────┘      └──────────────┘      │                   │
          ...
          <p className="text-xs text-gray-500 mt-2">
            No Mock API in production. The Engine calls real tenant APIs
            directly.
          </p>
        </Section>
...
# cd services/engine && npm run build     # esbuild → dist/server.js (~357KB)`}</CmdBlock>
        ...
        <Section title="2. Configure Environment (.env.prod)">
          <p className="text-sm text-gray-600 mb-3">
            Copy <Code>.env.example</Code> to <Code>.env.prod</Code> and fill in
            your production values. Both the UI and Engine services read from
            this file.
          </p>
...
cp .env.example .env.prod`}</CmdBlock>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                ...
                <tbody className="text-gray-700">
                  <tr>
                    <td className="px-3 py-1 font-mono">NODE_ENV</td>
                    <td className="px-3 py-1">production</td>
                    <td className="px-3 py-1">
                      Enables production optimizations, structured logging
                    </td>
                  </tr>
                  <tr className="bg-red-50/50">
                    <td className="px-3 py-1 font-mono">ENGINE_URL</td>
                    <td className="px-3 py-1">http://engine:4001</td>
                    <td className="px-3 py-1">
                      UI proxies API calls to Engine (Docker) or
                      http://localhost:4001 (PM2)
                    </td>
                  </tr>
                  <tr>
                    <td className="px-3 py-1 font-mono">USER_INFO_URL</td>
                    <td className="px-3 py-1">
                      https://sso.your-company.com/api/userinfo
                    </td>
                    <td className="px-3 py-1">
                      AD/SSO endpoint for user authentication
                    </td>
                  </tr>
                  <tr className="bg-red-50/50">
                    <td className="px-3 py-1 font-mono">API_BASE_URL</td>
                    <td className="px-3 py-1">
                      https://api.your-company.com/api
                    </td>
                    <td className="px-3 py-1">Tenant data API base URL</td>
                  </tr>
                  <tr>
                    <td className="px-3 py-1 font-mono">API_TOKEN</td>
                    <td className="px-3 py-1">your-prod-api-token</td>
                    <td className="px-3 py-1">
                      Global bearer token for APIs using bearer auth
                    </td>
                  </tr>
                  <tr className="bg-red-50/50">
                    <td className="px-3 py-1 font-mono">ENGINE_API_KEY</td>
                    <td className="px-3 py-1">your-secure-random-key</td>
                    <td className="px-3 py-1">
                      <strong>Required:</strong> Secures the Engine admin API
                      endpoints
                    </td>
                  </tr>
                  <tr>
                    <td className="px-3 py-1 font-mono">UI_ORIGIN</td>
                    <td className="px-3 py-1">
                      https://chatbot.your-company.com
                    </td>
                    <td className="px-3 py-1">
                      CORS allowed origin for Engine
                    </td>
                  </tr>
                  <tr className="bg-red-50/50">
                    <td className="px-3 py-1 font-mono">FILE_BASE_DIR</td>
                    <td className="px-3 py-1">{"//server/shared/reports"}</td>
                    <td className="px-3 py-1">
                      Base directory for CSV/XLSX file paths (shared network
                      folder)
                    </td>
                  </tr>
                </tbody>
          ...
          <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg text-xs text-amber-700">
            ...
            <Code>.env.prod</Code> to git — it contains secrets. Only{" "}
            <Code>.env.example</Code> (the template) is version-controlled.
          </div>
        </Section>

        <Section title="3. Per-Query Authentication">
          <p className="text-sm text-gray-600 mb-3">
            Each query independently specifies its authentication method.
            Configure per query in Admin &rarr; Groups &rarr; Queries.
          </p>
            ...
            <table className="w-full text-xs border border-gray-200 rounded">
              ...
              <tbody className="text-gray-700">
                ...
                <tr className="bg-gray-50">
                  ...
                  <td className="px-3 py-2 border-b">
                    Uses global <Code>API_TOKEN</Code> from env as
                ...
                <tr>
                  ...
                  <td className="px-3 py-2 border-b">
                    Forwards logged-in user&apos;s AD/Kerberos credentials from
                    the browser session
                  </td>
                  <td className="px-3 py-2 border-b font-mono">
                    authType: &quot;windows&quot;
                ...
                <tr className="bg-gray-50">
                  ...
                  <td className="px-3 py-2 border-b">
                    Calls BAM token URL first, then uses bamToken in X-BAM-Token
                    header for data call
                  </td>
                  <td className="px-3 py-2 border-b font-mono">
                    authType: &quot;bam&quot;, bamTokenUrl:
                    &quot;https://...&quot;
                  </td>
                </tr>
              </tbody>
  ...
  → Engine forwards user's cookies/Authorization header → Tenant API`}</pre>
              <p className="mt-1">
                No extra configuration per query — the user&apos;s existing AD
                session is forwarded transparently.
              </p>
            </div>
...
Engine → GET/POST data API with X-BAM-Token: <bamToken> → data`}</pre>
              <p className="mt-1">
                BAM tokens are cached (~5 min TTL). Set <Code>bamTokenUrl</Code>{" "}
                per query in the admin UI.
              </p>
            </div>
          </div>
        </Section>
        ...
        <Section title="4. Deploy with Docker">
          <p className="text-sm text-gray-600 mb-3">
            Uses <Code>docker-compose.prod.yml</Code> which reads from{" "}
            <Code>.env.prod</Code> automatically.
          </p>
...
pm2 startup`}</CmdBlock>
          <p className="text-xs text-gray-500">
            PM2 inherits environment variables from the shell session. Either
            source <Code>.env.prod</Code> before starting, or set system-wide
            env vars (see Windows Setup guide for PowerShell approach).
          </p>
        </Section>
    ...
    location /engine/ {
        ...
        proxy_set_header Host $host;
          ...
          <p className="text-xs text-gray-500">
            For Windows auth, ensure the reverse proxy forwards Authorization
            headers. For IIS setup, see the Windows Setup guide.
          </p>
        </Section>
...
npm run analyze        # Bundle analysis (interactive treemap)`}</pre>
        ...
        <Section title="ML Data Persistence">
          <p className="text-sm text-gray-600 mb-3">
            ML features store data under <Code>services/engine/data/</Code>. In
            production, ensure this directory is persistent across deployments.
          </p>
          <div className="overflow-x-auto">
            <table className="w-full text-xs border border-gray-200 rounded">
              <thead>
              ...
              <tbody className="text-gray-700">
                <tr>
                  <td className="px-3 py-1.5 border-b">Semantic Search</td>
                  <td className="px-3 py-1.5 border-b font-mono">
                    data/indexes/{"{groupId}/"}
                  </td>
                  <td className="px-3 py-1.5 border-b">
                    TF-IDF index for natural language query matching
                  </td>
                </tr>
                <tr className="bg-gray-50">
                  <td className="px-3 py-1.5 border-b">Recommendations</td>
                  <td className="px-3 py-1.5 border-b font-mono">
                    data/learning/{"{groupId}/"}
                  </td>
                  <td className="px-3 py-1.5 border-b">
                    User interactions, signal aggregates, collaborative filter
                    data
                  </td>
                </tr>
                <tr>
                  <td className="px-3 py-1.5 border-b">Anomaly Detection</td>
                  <td className="px-3 py-1.5 border-b font-mono">
                    data/anomaly/{"{groupId}/"}
                  </td>
                  <td className="px-3 py-1.5 border-b">
                    Historical baselines and snapshots for z-score/IQR checks
                  </td>
                </tr>
                <tr className="bg-gray-50">
                  <td className="px-3 py-1.5 border-b">User Preferences</td>
                  <td className="px-3 py-1.5 border-b font-mono">
                    data/preferences/
                  </td>
                  <td className="px-3 py-1.5 border-b">
                    Per-user favorites and settings
                  </td>
                </tr>
              </tbody>
            </table>
          ...
          <div className="mt-3 space-y-2">
            <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg text-xs text-amber-700">
              <span className="font-medium">Docker:</span> Mount{" "}
              <Code>services/engine/data/</Code> as a volume to persist ML data
              across container restarts:
              <pre className="font-mono mt-1 text-[10px]">{`volumes:
  - ./engine-data:/app/services/engine/data`}</pre>
            </div>
            <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg text-xs text-blue-700">
              <span className="font-medium">PM2:</span> Data persists on disk
              automatically. Back up <Code>services/engine/data/</Code>{" "}
              regularly.
            </div>
          </div>
        </Section>
          ...
          <div className="space-y-2 text-sm text-gray-600">
            ...
            <div className="flex items-start gap-2">
              ...
              <span>
                ...
                returns real AD identity (not mock user)
              </span>
            </div>
            <div className="flex items-start gap-2">
              <span className="text-gray-400">&#9744;</span>
              <span>
                Admin access: Only users in <Code>users.json</Code> with role
            ...
            <div className="flex items-start gap-2">
              <span className="text-gray-400">&#9744;</span>
              <span>
                Conversation logs: Check Admin &rarr; Conversation Logs for
                activity
              </span>
            </div>
            <div className="flex items-start gap-2">
            ...
            <div className="flex items-start gap-2">
              <span className="text-gray-400">&#9744;</span>
              <span>SSL certificate: Verify HTTPS works end-to-end</span>
            </div>
            <div className="flex items-start gap-2">
              <span className="text-gray-400">&#9744;</span>
              <span>
                ENGINE_API_KEY: Verify{" "}
                <code className="text-xs bg-gray-100 px-1 rounded">
                  GET /api/admin/*
                </code>{" "}
