...
export default function IntentBuilderPage() {
  ...
  const [deleteIntentTarget, setDeleteIntentTarget] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    if (
      selectedEntity &&
      selectedOption &&
      entities[selectedEntity]?.options[selectedOption]
    ) {
      setEditSynonyms(
        entities[selectedEntity].options[selectedOption].join("\n"),
      );
    }
  ...
  const handleSaveIntent = async () => {
    ...
    setSaving(true);
    try {
      const utterances = editUtterances
        .split("\n")
        .map((u) => u.trim())
        .filter(Boolean);
      const answers = editAnswers
        .split("\n---\n")
        .map((a) => a.trim())
        .filter(Boolean);
  ...
  const handleCreateIntent = async () => {
    ...
    setSaving(true);
    try {
      const utterances = newIntentUtterances
        .split("\n")
        .map((u) => u.trim())
        .filter(Boolean);
      await fetch("/api/admin/intents", {
  ...
  const handleDeleteIntent = async (intent: string) => {
    await fetch(`/api/admin/intents?intent=${encodeURIComponent(intent)}`, {
      method: "DELETE",
      headers: { ...csrfHeaders() },
    });
  ...
  const handleAddEntityOption = async () => {
    if (!selectedEntity || !newOptionKey.trim() || !newOptionSynonyms.trim())
      return;
    setSaving(true);
    try {
      const synonyms = newOptionSynonyms
        .split("\n")
        .map((s) => s.trim())
        .filter(Boolean);
      await fetch("/api/admin/entities", {
  ...
  if (loading)
    return <p className="text-sm text-gray-500">Loading intents...</p>;
  ...
  return (
    <div>
      <div className="pb-6 mb-6 border-b border-gray-100">
        <h1 className="text-2xl font-semibold text-gray-900">Intent Builder</h1>
        <p className="text-sm text-gray-500">
          Manage intents, training phrases, and entity definitions
        </p>
      </div>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
                ...
                <div className="mb-3">
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    Use @entity_name to reference entities (e.g., @query_name,
                    @time_period)
                  </p>
                </div>
                ...
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <span className="text-xs text-gray-400">
                      {selected.utteranceCount} training phrases
                    </span>
                  </div>
                  <button
                ...
                <div className="mb-4">
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <input
                      ...
                      onKeyDown={(e) =>
                        e.key === "Enter" && handleAddUtterance()
                      }
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    Use @entity_name for entity references. One phrase per line.
                  </p>
                </div>
                ...
                <div className="mb-4">
                  <label className="block text-xs font-medium text-gray-500 mb-1">
                    ...
                    <span className="text-gray-400 font-normal">
                      Optional: used for static intents like greeting/help
                    </span>
                  </label>
                  <textarea
                    value={editAnswers}
      ...
      {activeTab === "entities" && (
        <div className="flex gap-4">
          ...
          {selectedEntity ? (
            <>
              <div className="w-52 shrink-0">
                ...
                <div className="space-y-1 max-h-96 overflow-y-auto">
                  {Object.entries(entities[selectedEntity]?.options || {}).map(
                    ([key, synonyms]) => (
                      <button
                        key={key}
                      ...
                      >
                        ...
                        <div className="text-[10px] text-gray-400">
                          {(synonyms as string[]).length} synonyms
                        </div>
                      </button>
                    ),
                  )}
            ...
            <div className="flex-1 bg-white rounded-xl shadow-sm border border-gray-200 p-8 text-center">
              <p className="text-sm text-gray-500">
                Select an entity type from the list to view and edit its
                options.
              </p>
            </div>
          )}
      ...
      <ConfirmModal
        ...
        message={
          deleteIntentTarget
            ? `Delete intent "${deleteIntentTarget}"? This cannot be undone.`
            : ""
        }
        confirmLabel="Delete"
        variant="danger"
        onConfirm={() => {
          if (deleteIntentTarget) handleDeleteIntent(deleteIntentTarget);
        }}
