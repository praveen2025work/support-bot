...
self.onmessage = async (event: MessageEvent<WorkerRequest>) => {
  ...
  try {
    const data = JSON.parse(payload.csvData) as Record<string, (string | number | null)[]>;
    ...
    switch (type) {
      ...
      case 'cluster': {
        ...
        result = clusterData(
          numericData,
          opts.columns as string[],
          kmeans as unknown as (data: number[][], k: number, options?: object) => { clusters: number[]; centroids: number[][] },
