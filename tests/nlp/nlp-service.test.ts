...
const FAQ_DATA = [
  {
    ...
    intent: 'url.find',
    answer: 'You can access the monthly revenue report. Would you like me to find the direct link?',
  },
  {
...
describe('NlpService', () => {
  ...
  describe('initialization', () => {
    ...
    it('throws NlpNotInitializedError when classify is called before init', async () => {
      ...
      await expect(uninitialised.classify('hello')).rejects.toThrow('NLP service not initialized');
    });
  });
  ...
  describe('confidence threshold', () => {
    it('returns classification result with source field', async () => {
      ...
      expect(['nlp', 'fuzzy']).toContain(result.source);
    });
  ...
  describe('unknown input', () => {
    it('returns None intent for completely unrelated input', async () => {
      const result = await service.classify('xyzzy foobar quantum tractor');
      expect(result.intent).toBe('None');
      // nlpjs may return confidence of 1 for None intent (its default behaviour)
      // The key assertion is that intent is 'None'
    });

    it('returns None intent for random characters', async () => {
  ...
  describe('entity extraction', () => {
    it('extracts query_name entity from "run the monthly_revenue query"', async () => {
      ...
      const queryEntity = result.entities.find((e) => e.entity === 'query_name');
      if (queryEntity) {
    ...
    it('extracts time_period entity from "run monthly revenue for this month"', async () => {
      const result = await service.classify('run monthly revenue for this month');
      const timePeriod = result.entities.find((e) => e.entity === 'time_period');
      if (timePeriod) {
    ...
    it('extracts date entities like "Jan 2026" via date-entity-extractor', async () => {
      const result = await service.classify('show me monthly revenue for Jan 2026');
      const timePeriod = result.entities.find((e) => e.entity === 'time_period');
  ...
  describe('sentiment analysis', () => {
    it('includes sentiment data in classification result', async () => {
      ...
      if (result.sentiment) {
        ...
        expect(['positive', 'neutral', 'negative']).toContain(result.sentiment.vote);
      }
    });
  });
