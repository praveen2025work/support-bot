/**
 * Connection Manager — singleton managing MSSQL connector instances.
 */
...
class ConnectionManager {
  ...
  async getConnector(connectorId: string): Promise<IDatabaseConnector> {
    ...
    if (!config) throw new Error(`SQL connector not found: ${connectorId}`);
    ...
    const connector = new MssqlConnector(config, password || "");
  ...
  async shutdown(): Promise<void> {
    ...
    for (const [id, connector] of this.connectors) {
      logger.info({ connectorId: id }, "Shutting down MSSQL connector");
      promises.push(connector.close());
    }
    await Promise.allSettled(promises);
    this.connectors.clear();
    logger.info("All MSSQL connectors shut down");
  }
}
