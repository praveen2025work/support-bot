...
export function parseFilterFromText(
  ...
  if (
    GROUP_BY_PATTERN.test(text) ||
    SORT_PATTERN.test(text) ||
    SUMMARY_PATTERN.test(text)
  ) {
    return null;
  }
  ...
  for (const [alias, canonical] of Object.entries(filterAliases)) {
    ...
    const byPattern = new RegExp(
      `\\b(?:filter|by|with|in|for|=)\\s*${alias}\\s+(?:=\\s*)?([\\w]+)`,
      "i",
    );
...
/**
 * Extract query-specific filters from user text using the query's defined filter keys.
 * This handles custom filters (e.g. "status", "orderDate") that aren't in the
 * hardcoded NLP entity list or filterAliases.
 *
 * Patterns recognized:
 *   "run order_details status Completed"       → { status: "Completed" }
 *   "run order_details status=Completed"       → { status: "Completed" }
 *   "status Completed orderDate 2024-01-15"    → { status: "Completed", orderDate: "2024-01-15" }
 *   "filter by status Completed"               → { status: "Completed" }
 */
export function extractQuerySpecificFilters(
  text: string,
  queryFilterKeys: string[],
): Record<string, string> {
  if (!queryFilterKeys.length) return {};

  const filters: Record<string, string> = {};

  for (const key of queryFilterKeys) {
    // Skip keys already handled by the standard parser
    const standardKeys = [
      "region",
      "team",
      "environment",
    ...
    // Pattern 1: key=value (e.g. "status=Completed")
    const equalsMatch = new RegExp(
      `\\b${escapeRegex(key)}\\s*=\\s*([^\\s,]+)`,
      "i",
    ).exec(text);
    if (equalsMatch) {
      filters[key] = equalsMatch[1];
      continue;
    }

    // Pattern 2: key value (e.g. "status Completed")
    // Value is the next non-whitespace token after the key
    const spaceMatch = new RegExp(
      `\\b${escapeRegex(key)}\\s+([^\\s,]+)`,
      "i",
    ).exec(text);
    if (spaceMatch) {
      // Don't capture common noise words as values
      const noiseWords = [
        "by",
        "is",
        "for",
      ...
      if (!noiseWords.includes(spaceMatch[1].toLowerCase())) {
        filters[key] = spaceMatch[1];
        continue;
      }
    }
  }
