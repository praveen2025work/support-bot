...
function extractColumnFromText(text: string, headers: string[]): string | null {
  ...
  const ofMatch = text.match(
    /\b(?:of|for|in|on)\s+(\w[\w\s]*?)(?:\s*$|\s+(?:column|field|data))/i,
  );
  if (ofMatch) {
    const term = ofMatch[1].trim().toLowerCase();
    for (const h of headers) {
      if (h.toLowerCase() === term) return h;
      if (h.toLowerCase().replace(/[_\s]/g, "") === term.replace(/[_\s]/g, ""))
        return h;
    }
  }
  ...
  for (const word of words.reverse()) {
    for (const h of headers) {
      ...
      if (h.toLowerCase().replace(/[_\s]/g, "") === word.replace(/[_\s]/g, ""))
        return h;
    }
  }
  return null;
  ...
  try {
    switch (intent) {
      ...
      case INTENTS.ANALYSIS_DECISION_TREE:
        return await handleDecisionTree(
          csvData,
          userText,
          context,
          classification,
...
async function handleDistribution(
  ...
  if (!column) {
    ...
    const numCols = csvData.headers.filter((h) => {
      ...
      return sample.some(
        (r) => typeof r[h] === "number" || !isNaN(parseFloat(String(r[h]))),
      );
    });
    if (numCols.length === 0) {
      return {
...
async function handleAnomaly(
  ...
  const numericCols = csvData.headers.filter((h) => {
    ...
    for (const row of sample) {
      ...
      if (
        typeof v === "number" ||
        (typeof v === "string" && !isNaN(parseFloat(v)) && v.trim() !== "")
      )
        numCount++;
    }
...
async function handleForecast(
  ...
  const periodsMatch = userText.match(
    /\b(\d+)\s*(?:periods?|months?|weeks?|days?|steps?|ahead)?\b/i,
  );
  const periods = periodsMatch ? parseInt(periodsMatch[1], 10) : 5;
  const valueCol = extractColumnFromText(userText, csvData.headers);
  const result = ml.forecast(
    csvData,
    periods,
    undefined,
    valueCol ?? undefined,
...
async function handlePCA(
  ...
  const totalVar = (
    (result.varianceExplained[0] + result.varianceExplained[1]) *
    100
  ).toFixed(1);
  return {
...
async function handleReport(
  ...
  const report = ml.compileInsightReport(csvData, {
    profiles,
    summary,
    missing,
  });
